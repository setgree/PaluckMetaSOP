...
=======
Third, we detail our meta-analytic defaults. For instance, we use a random effects model (as implemented by the `metafor` package in R [@viechtbauer2010]) rather than a fixed effects (sometimes called an equal effects) model where we cluster at the level of an individual study. Further, we prefer Glass's ∆ to Cohen's _d_ so as to avoid an additional assumption about the effects of treatment on the underlying distribution. Here, we also detail a novel estimator for converting difference in proportion
>>>>>>> 965797f8b7cefaf2eb25de39fb4250583040c662
