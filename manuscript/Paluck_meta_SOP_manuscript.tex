...
Finally, we conclude by noting some open questions in our vision of meta-scientific meta-analyses. Fundamentally, these issues boil down to the unclear line between \emph{deduction} and \emph{induction} in meta-analysis, i.e.~the extent to which such papers should be tests of initial hypotheses or instead be guided by discoveries in the data. In practical, a meta-analysis is both, but this can make a task like writing a pre-analysis plan tricky if the most relevant empirical questions become clear only after reading a representative sample of papers, which is equivalent to initial data collection. Likewise with the question of which outcome or cluster of outcomes to focus on collecting and analyzing, which can be difficult to tell prior to data collection. Last, discuss the difficulty of integrating indirect cues of study quality, e.g.~obviously mistaken or implausible statistical results on one end or special care to reproducibility on the other, into our analyses in a principled way.
