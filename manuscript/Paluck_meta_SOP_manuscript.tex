...
\abstract{%
This paper describes the motivations and procedures for meta-analyses by Betsy Levy Paluck and co-authors, with reference to and examples drawn from three such papers (Paluck, Green, \& Green, 2019; Paluck, Porat, Clark, \& Green, 2021; Porat, Gantman, Paluck, Green, \& Pezzuto, 2024). We first describe how our meta-analyses advance the meta-scientific goals of critically assessing the robustness of a literature's core findings with respect to differences in study design, measurement strategy, intervention approach, and estimand. We also detail where we think quantitative reviews add value and how we think through the trade-off between comprehensive and selective inclusion criteria. Second, we describe the typical `flow' of the papers as a whole. Third, we describe our meta-analytic procedures, detailing our default analytic choices and introducing an R package that implements those choices. These `Standard Operating Procedures' are intended as a guide to a handful of especially thorny meta-analytic issues. We conclude with some open questions for meta-analysts.
}
...
\section{The trade-off between comprehensiveness and narrow, focused reviews}\label{the-trade-off-between-comprehensiveness-and-narrow-focused-reviews}
...
We take a different tack: we drop the assumption that a pooled meta-analytic effect should be considered causal. Instead, when we combine studies from heterogeneous designs, treatments, and outcomes, the pooled estimate is a \emph{descriptive statistic} representing the average size of changes measured by a group of researchers, according to them, with outcomes they chose. If all the meta-analytic estimates in our papers were placed on a spectrum of credulousness, this would be on the most credulous end: it tells us how a field is doing at its main task according to itself. For Paluck et al. (2021), this number represented the average effects found by prejudice researchers between 2007 and 2019; for Porat et al. (2024), the average effects found by sexual violence researchers between 1986 and 2018.
...
A comprehensive meta-analysis allows the researcher to furnish several meta-scientific quanitites

\subsubsection{Publication bias}\label{publication-bias}

Tests for publication bias are more informative if they encompass the entire literature. Classical approaches to assessing publication bias in meta-analysis are ``based on the fact that precision in estimating the underlying treatment effect will increase as the sample size of component studies increases'' (Egger, Smith, Schneider, \& Minder, 1997). The funnel plot detects publication bias under the assumption that smaller, more imprecisely estimated studies that produce null or backlash results are more likely to be shelved than large, precisely estimated studies that produce the same.\footnote{The underlying theory of researcher behavior seems to be a kind of widespread acceptance of the sunk cost fallacy: once you've invested the kind of time and energy that a large, rigorous study requires, you'll push through to publication even if you'd put the results back in the `file drawer' (Rosenthal, 1979) if you'd put less work in.} (Having said that, one can simply plot the relationship between effect size and standard error, as we did in Paluck et al. (2019), where the strong positive relationship between the two suggested that ``a very large study would be expected to produce a minuscule \emph{increase} in prejudice.'' These tests require estimates of effect size and variance for each study.

Estimates of publication bias can also be interesting between meta-analyses. For instance, we found strong evidence of publication bias in Paluck et al. (2019) and Paluck et al. (2021) but not in Porat et al. (2024), which drew more from public health perspectives and departments than did the previous two papers. This kind of finding sheds light on whether, and along which dimensions, the credibility revolution is taking hold across disciplines.

\subsubsection{Comparisons across intervention strategies}\label{comparisons-across-intervention-strategies}
...
\subsubsection{Robustness to differences in study design}\label{robustness-to-differences-in-study-design}

Third, relationships between effect and study design, measured in a variety of ways, are often illuminating. The most basic of these is whether studies were randomized or not. Porat et al. (2024), for example, finds a pooled effect size of 0.503 within observational designs, 0.287 for quasi-experiments, and 0.307 for randomized controlled trials. Another candidate is statistical power: Paluck et al. (2021) found a highly significant relationship between sample size and effect size, with the smallest quintile of studies producing an average effect of \emph{d} = 0.61 vs \emph{d} = 0.19 for the largest quintile. Last, researchers can check for robustness to adherence to open science practices; Paluck et al. (2019), for instance, found that studies with pre-analysis plans had an average effect size of 0.016, vs.~0.451 for everything else.

We wish to emphasize that these are \emph{not} well-identified estimates of the `effect' of study designs, sample sizes, or pre-analysis plans \textemdash none of these things were randomly assigned, and many other things varied between these studies \textemdash but it is at least a `hoop test' (Collier, 2011) that something is amiss, e.g, that p-hacking is a problem in the contact hypothesis literature.

\subsubsection{Robustness to differences in measurement strategy}\label{robustness-to-differences-in-measurement-strategy}

Fourth, quantitative estimates allow for assessing differences in effect size between outcomes and how they're measured. This can be interesting both within literatures and across them. The three meta-analyses discussed in this paper mostly measure attitudes, and their overall effect sizes all fall between 0.28 and 0.357: a relatively small range. However, for Porat et al. (2024), the key quantity of interest \textemdash sexual violence \textemdash is fundamentally a behavioral problem. There, our results are discouraging: pooled effect sizes of just 0.033 for perpetration outcomes and 0.046 for victimization, neither of which is statistically significant.\footnote{Likewise, Green, Mathur, and Smith (2024) look at interventions intended to reduce consumption of meat and animal products and only meta-analyze direct measures of what people eat \textemdash rather than what they think, feel or intend to do about what they eat \textemdash and find an average effect size of 0.131.} Another useful check is for whether delayed outcome measurement predicts effect sizes; in Paluck et al. (2021), we found an overall mixed relationship between the two variables, but a general preponderance of immediate measurement made this comparison difficult.

First, coding all available studies allows for the most comprehensive possible assessments of publication bias, an essential meta-scientific quantity.

Second, any difference in effect sizes between randomized and non-randomized interventions allows researchers to put a parameter value on the potential scope of confounding bias in the non-randomized literature. If the effect sizes are basically the same, that suggests that confounding is not a significant issue. On the other hand, if randomized interventions produce effect sizes that are on average \emph{d} = 0.2 smaller, this suggests that future observational work can be `discounted' by approximately that amount. This is obviously not a foolproof estimation strategy, but we think it's good start for for establishing a quantitative prior.
