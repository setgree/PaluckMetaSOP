...
2. combine papers that are drawing from vastly different theoretical traditions, e.g., within the sexual violence literature, a building-based intervention comprising ``higher levels of faculty/security presence in safe/unsafe `hot spots' mapped by students'' (Taylor, Stein, Mumford, \& Woods, 2013), and a module of peer-based consent training on a college campus (Crane, 2017), which are not obviously testing the same thing
3. mixed widely divergent dependent variables, e.g.~again with the sexual violence literature, where we coded the Sexual Experiences Survey Koss \& Gidycz (1985) for behavioral outcomes and the Illinois Rape Myth Acceptance Scale (Thelan \& Meadows, 2022) for ideas-based outcomes when they were available, but the nearest substitute when they weren't.
