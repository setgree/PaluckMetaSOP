...
A healthy literature has interrogated meta-analytic methods, noting common pitfalls (Greco, Zangrillo, Biondi-Zoccai, \& Landoni, 2013), statistical errors (Kadlec, Sainani, \& Nimphius, 2023), and practical limitations, e.g.~a widespread lack of reproducibility (Daniel Lakens et al., 2017) or data availability (\textbf{CITE?}). This paper sits in that tradition, but is a bit of an offshoot in that we are not attempting to correct statistical or practical mistakes in meta-analyses, but rather articulate what it is that we are trying to do when we write them. In this regard, this paper is closely in dialogue with two recent contributions that attempt to put meta-analysis on firmer theoretical footing.
...
\subsection{Meta-scientific quantities illuminiated by meta-analysis}\label{meta-scientific-quantities-illuminiated-by-meta-analysis}
