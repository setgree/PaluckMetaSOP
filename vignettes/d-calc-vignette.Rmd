...
We take the latest possible result (Middle of Third Year) and include the roommate assigned in first year (in a world where roommate assignment reduces prejudice, we would expect to see this partly manifesting as likelihood of white students' still being friends with their initially assigned black roommates). So our `stat` is `t = -3.705`, which we convert to an SMD estimate via $d = t * \sqrt\frac{n_t + n_c}{n_t * n_c}$. We take our Ns from that table as well: `n_t = 44` and `n_c = 214`.
...
When we are comparing two variances, the F testis equivalent to the square of the t-test. The conversion formula is the same as that which converts a t-test to ∆, except with the F inside the square root:
