...
`map_robust` is a wrapepr around a pre-existing function from the `metafor` package called `robust`, which is itself a wrapped around the function `rma`. See the `man` pages for those functions for full details.
...
Next up: [Writing your meta-analytic paper](writing-metas-vignette.html).
