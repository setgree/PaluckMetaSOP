---
title: "03: Performing meta-analysis the Paluck Lab way"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{03: Performing meta-analysis the Paluck Lab way}
...
`map_robust` takes a dataset and returns a tibble with an N of the number of data points; an N for the number of studies; a ∆ signifying the pooled effect size; a standard error; and a p value.
...
```{r split_plus_kable}
library(knitr)
sv_data |> split(~behavior_type) |> map(map_robust) |> bind_rows() |> kable('markdown')
```
