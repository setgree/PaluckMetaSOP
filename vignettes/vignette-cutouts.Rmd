...
### 1.3 Solution: standardized mean difference (SMD)

A **standardized mean difference** (SMD) is an estimator that addresses these problems. It takes the following form:

$$
\frac{M1 - M2}{SD}
$$

It represents the difference in means between two groups divided by the standard deviation (SD) of the outcome. An SMD tells you how many standard deviations worth of change an intervention produces.

See [this page on SMDs](https://handbook-5-1.cochrane.org/chapter_9/9_2_3_2_the_standardized_mean_difference.htm) from the Cochrane collaboration for further reading.

### 1.4 Cohen's d vs. Glass's ∆

If you standardize your effect size with an estimated SD of the outcome for entire population, your estimator is called [Cohen's d](https://resources.nu.edu/statsresources/cohensd). If you use an estimate SD from just the control group, you get [Glass's ∆](https://www.statisticshowto.com/glasss-delta/) (Delta).

The Paluck lab generally prefers Glass's ∆ because we're interested in the efficacy of a given intervention relative to the population's baseline state, i.e. how they thought, felt, or behaved before they got the intervention. When the SD of the control group isn't available, ewe use the SD of the entire population. When neither is available, or can't be figured out from the statistics a paper presents, we have a problem (we'll get to that later).

For the rest of this vignette, we'll be talking about Glass's ∆.
...
## 2. Calculating Delta, variance, and standard error from information presented in studies.

Studies present their results in many different ways! Some of these are really easy to convert to an SMD, some are harder, and some are actually impossible. This section walks through some examples in each category and shows you how you can use the `d_calc` and `var_d_calc` to calculate ∆, variance, and standard error reproducibly.
